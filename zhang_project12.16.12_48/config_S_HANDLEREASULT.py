...
c.execute("select I_SECURITYID,S_CHECKAREA,S_RISKPOINT,S_HANDLEREASULT,D_CHECKDATE,S_PROBLEMDESCRIPTION,S_MODIFYMEASURE from S_SECURITY_CHECK where S_RISKPOINT LIKE 'T137%'")
column_name1 = c.fetchall()
show_table_DF = pd.DataFrame(column_name1,columns=['I_SECURITYID','S_CHECKAREA','S_RISKPOINT','S_HANDLEREASULT','D_CHECKDATE','S_PROBLEMDESCRIPTION','S_MODIFYMEASURE'])
# print(show_table_DF)
...
#########################################################################################
# 展示发卡类别计数
def show_handler_result_value_counts():
    return handler_result_value_counts
